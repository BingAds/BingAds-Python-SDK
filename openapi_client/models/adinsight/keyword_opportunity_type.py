...
class KeywordOpportunityType(Flag):
    DEFAULTVALUE = auto()

    BROADMATCH = auto()

    CAMPAIGNCONTEXT = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            KeywordOpportunityType.DEFAULTVALUE: 'DefaultValue',
            KeywordOpportunityType.BROADMATCH: 'BroadMatch',
            KeywordOpportunityType.CAMPAIGNCONTEXT: 'CampaignContext',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in KeywordOpportunityType if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "KeywordOpportunityType":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid KeywordOpportunityType value: {value}")
        raise ValueError(f"Cannot convert {value} to KeywordOpportunityType")
