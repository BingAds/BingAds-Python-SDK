...
class KeywordAdditionalField(Flag):
    NONE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            KeywordAdditionalField.NONE: 'None',
            KeywordAdditionalField.MAXCONVERSIONVALUEWITHMAXCPC: 'MaxConversionValueWithMaxCpc',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in KeywordAdditionalField if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "KeywordAdditionalField":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid KeywordAdditionalField value: {value}")
        raise ValueError(f"Cannot convert {value} to KeywordAdditionalField")
