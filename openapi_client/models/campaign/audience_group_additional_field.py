...
class AudienceGroupAdditionalField(Flag):
    NONE = auto()

    PROFILEDIMENSION = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AudienceGroupAdditionalField.NONE: 'None',
            AudienceGroupAdditionalField.PROFILEDIMENSION: 'ProfileDimension',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AudienceGroupAdditionalField if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AudienceGroupAdditionalField":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AudienceGroupAdditionalField value: {value}")
        raise ValueError(f"Cannot convert {value} to AudienceGroupAdditionalField")
