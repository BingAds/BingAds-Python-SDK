...
class HotelAdGroupType(Flag):
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            HotelAdGroupType.HOTELAD: 'HotelAd',
            HotelAdGroupType.PROPERTYAD: 'PropertyAd',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in HotelAdGroupType if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "HotelAdGroupType":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid HotelAdGroupType value: {value}")
        raise ValueError(f"Cannot convert {value} to HotelAdGroupType")
