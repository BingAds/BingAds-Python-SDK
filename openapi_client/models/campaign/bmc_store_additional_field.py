...
class BMCStoreAdditionalField(Flag):
    NONE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            BMCStoreAdditionalField.NONE: 'None',
            BMCStoreAdditionalField.GLOBALSTORE: 'GlobalStore',
            BMCStoreAdditionalField.STOREURL: 'StoreUrl',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in BMCStoreAdditionalField if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "BMCStoreAdditionalField":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid BMCStoreAdditionalField value: {value}")
        raise ValueError(f"Cannot convert {value} to BMCStoreAdditionalField")
