...
class ConversionGoalAdditionalField(Flag):
    NONE = auto()

    VIEWTHROUGHCONVERSIONWINDOWINMINUTES = auto()

    ISEXTERNALLYATTRIBUTED = auto()

    GOALCATEGORY = auto()

    INACTIVEDUETOTAGUNAVAILABLE = auto()

    ATTRIBUTIONMODELTYPE = auto()
    ...
    @classmethod
    def _validate(cls, value, handler) -> "ConversionGoalAdditionalField":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid ConversionGoalAdditionalField value: {value}")
        raise ValueError(f"Cannot convert {value} to ConversionGoalAdditionalField")
