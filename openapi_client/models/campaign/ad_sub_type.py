...
class AdSubType(Flag):
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            ...
            AdSubType.DISPLAY: 'Display',
            AdSubType.HTML5: 'HTML5',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AdSubType if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AdSubType":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AdSubType value: {value}")
        raise ValueError(f"Cannot convert {value} to AdSubType")
