...
class InsertionOrderAdditionalField(Flag):
    NONE = auto()

    UNLIMITEDANDENDLESSFLAGS = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            InsertionOrderAdditionalField.NONE: 'None',
            InsertionOrderAdditionalField.UNLIMITEDANDENDLESSFLAGS: 'UnlimitedAndEndlessFlags',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in InsertionOrderAdditionalField if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "InsertionOrderAdditionalField":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid InsertionOrderAdditionalField value: {value}")
        raise ValueError(f"Cannot convert {value} to InsertionOrderAdditionalField")
