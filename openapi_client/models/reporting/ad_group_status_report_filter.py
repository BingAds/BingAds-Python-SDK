...
class AdGroupStatusReportFilter(Flag):
    ACTIVE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AdGroupStatusReportFilter.ACTIVE: 'Active',
            AdGroupStatusReportFilter.DELETED: 'Deleted',
            AdGroupStatusReportFilter.EXPIRED: 'Expired',
            AdGroupStatusReportFilter.PAUSED: 'Paused',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AdGroupStatusReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AdGroupStatusReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AdGroupStatusReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to AdGroupStatusReportFilter")
