...
class BidStrategyTypeReportFilter(Flag):
    MANUALCPC = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            BidStrategyTypeReportFilter.MANUALCPC: 'ManualCpc',
            BidStrategyTypeReportFilter.ENHANCEDCPC: 'EnhancedCpc',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in BidStrategyTypeReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "BidStrategyTypeReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid BidStrategyTypeReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to BidStrategyTypeReportFilter")
