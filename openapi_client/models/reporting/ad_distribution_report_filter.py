...
class AdDistributionReportFilter(Flag):
    SEARCH = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AdDistributionReportFilter.SEARCH: 'Search',
            AdDistributionReportFilter.AUDIENCE: 'Audience',
            AdDistributionReportFilter.CROSSNETWORK: 'CrossNetwork',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AdDistributionReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AdDistributionReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AdDistributionReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to AdDistributionReportFilter")
