...
class AdStatusReportFilter(Flag):
    ACTIVE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AdStatusReportFilter.ACTIVE: 'Active',
            AdStatusReportFilter.REJECTED: 'Rejected',
            AdStatusReportFilter.DELETED: 'Deleted',
            AdStatusReportFilter.PENDING: 'Pending',
            AdStatusReportFilter.PAUSED: 'Paused',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AdStatusReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AdStatusReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AdStatusReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to AdStatusReportFilter")
