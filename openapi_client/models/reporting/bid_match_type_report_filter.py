...
class BidMatchTypeReportFilter(Flag):
    EXACT = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            BidMatchTypeReportFilter.EXACT: 'Exact',
            BidMatchTypeReportFilter.PHRASE: 'Phrase',
            BidMatchTypeReportFilter.BROAD: 'Broad',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in BidMatchTypeReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "BidMatchTypeReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid BidMatchTypeReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to BidMatchTypeReportFilter")
