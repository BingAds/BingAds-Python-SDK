...
class DeviceTypeReportFilter(Flag):
    COMPUTER = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            DeviceTypeReportFilter.COMPUTER: 'Computer',
            DeviceTypeReportFilter.SMARTPHONE: 'SmartPhone',
            DeviceTypeReportFilter.NONSMARTPHONE: 'NonSmartPhone',
            DeviceTypeReportFilter.TABLET: 'Tablet',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in DeviceTypeReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "DeviceTypeReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid DeviceTypeReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to DeviceTypeReportFilter")
