...
class KeywordStatusReportFilter(Flag):
    ACTIVE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            KeywordStatusReportFilter.ACTIVE: 'Active',
            KeywordStatusReportFilter.PAUSED: 'Paused',
            KeywordStatusReportFilter.DELETED: 'Deleted',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in KeywordStatusReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "KeywordStatusReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid KeywordStatusReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to KeywordStatusReportFilter")
