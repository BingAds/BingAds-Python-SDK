...
class AssetGroupStatusReportFilter(Flag):
    ACTIVE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AssetGroupStatusReportFilter.ACTIVE: 'Active',
            AssetGroupStatusReportFilter.PAUSED: 'Paused',
            AssetGroupStatusReportFilter.DELETED: 'Deleted',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AssetGroupStatusReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AssetGroupStatusReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AssetGroupStatusReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to AssetGroupStatusReportFilter")
