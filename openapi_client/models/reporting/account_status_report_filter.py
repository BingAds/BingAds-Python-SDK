...
class AccountStatusReportFilter(Flag):
    ACTIVE = auto()
    ...
    def _to_str(self) -> str:
        _NAME_MAPPING = {
            AccountStatusReportFilter.ACTIVE: 'Active',
            AccountStatusReportFilter.PAUSED: 'Paused',
            AccountStatusReportFilter.INACTIVE: 'Inactive',
        }
        """Convert the enum flags to a comma-separated string of quoted, capitalized names"""
        names = [_NAME_MAPPING[flag] for flag in AccountStatusReportFilter if flag & self]
        if not names:
            return "None"
        if len(names) == 1:
    ...
    @classmethod
    def _validate(cls, value, handler) -> "AccountStatusReportFilter":
        ...
        if isinstance(value, str):
            ...
            if len(types) > 1:
                ...
                try:
                    ...
                    raise ValueError(f"Invalid AccountStatusReportFilter value: {value}")
        raise ValueError(f"Cannot convert {value} to AccountStatusReportFilter")
