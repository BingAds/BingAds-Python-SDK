def output_account(data_object):
    if data_object is None:
        return
    output_status_message("Account (Data Object):")
    output_status_message("AccountType: {0}".format(data_object.AccountType))
    ...
    output_status_message("CurrencyType: {0}".format(data_object.CurrencyType))
    ...
    if data_object.AccountType == 'AdvertiserAccount':
        output_advertiseraccount(data_object)

def output_array_of_account(data_objects):
    if data_objects is None or len(data_objects) == 0:
        return
    output_status_message("Array Of Account:\n")
    for data_object in data_objects['Account']:
        output_account(data_object)
...
def output_user(data_object):
    ...
    output_status_message("CustomerAppScope: {0}".format(data_object.CustomerAppScope))
    output_status_message("CustomerId: {0}".format(data_object.CustomerId))
    output_status_message("Id: {0}".format(data_object.Id))
    ...
    output_status_message("IsMigratedToMicrosoftAccount: {0}".format(data_object.IsMigratedToMicrosoftAccount))
...
def output_userinvitation(data_object):
    ...
    output_status_message("Email: {0}".format(data_object.Email))
    output_status_message("CustomerId: {0}".format(data_object.CustomerId))
    output_status_message("Role: {0}".format(data_object.Role))
...
def output_applicationtype(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_applicationtype(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of ApplicationType:\n")
    for value_set in value_sets['ApplicationType']:
        output_applicationtype(value_set)

def output_accounttype(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_accounttype(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of AccountType:\n")
    for value_set in value_sets['AccountType']:
        output_accounttype(value_set)

def output_currencytype(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_currencytype(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of CurrencyType:\n")
    for value_set in value_sets['CurrencyType']:
        output_currencytype(value_set)
...
def output_userrole(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_userrole(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of UserRole:\n")
    for value_set in value_sets['UserRole']:
        output_userrole(value_set)
