...
def output_nonhourlyreportaggregation(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_nonhourlyreportaggregation(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of NonHourlyReportAggregation:\n")
    for value_set in value_sets['NonHourlyReportAggregation']:
        output_nonhourlyreportaggregation(value_set)
...
def output_budgetsummaryreporttimeperiod(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_budgetsummaryreporttimeperiod(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of BudgetSummaryReportTimePeriod:\n")
    for value_set in value_sets['BudgetSummaryReportTimePeriod']:
        output_budgetsummaryreporttimeperiod(value_set)
...
def output_searchqueryreportaggregation(value_set):
    output_status_message("Values in {0}".format(value_set.Type))
    for value in value_set['string']:
        output_status_message(value)

def output_array_of_searchqueryreportaggregation(value_sets):
    if value_sets is None or len(value_sets) == 0:
        return
    output_status_message("Array Of SearchQueryReportAggregation:\n")
    for value_set in value_sets['SearchQueryReportAggregation']:
        output_searchqueryreportaggregation(value_set)
