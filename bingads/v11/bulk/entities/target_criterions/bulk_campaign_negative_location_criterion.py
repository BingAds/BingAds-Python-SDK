...
class BulkCampaignNegativeLocationCriterion(_SingleRecordBulkEntity):
    ...
    * :class:`.BulkFileWriter`
    ...
    _MAPPINGS = [
        ...
        _SimpleBulkMapping(
            _StringTable.Target,
            field_to_csv=lambda c: field_to_csv_LocationTarget(c.campaign_criterion),
            csv_to_field=lambda c, v: csv_to_field_LocationTarget(c.campaign_criterion, long(v) if v else None)
        ),
        _SimpleBulkMapping(
            _StringTable.SubType,
            field_to_csv=lambda c: field_to_csv_LocationType(c.campaign_criterion),
            csv_to_field=lambda c, v: csv_to_field_LocationType(c.campaign_criterion, v)
        ),
        _SimpleBulkMapping(
            _StringTable.Name,
            field_to_csv=lambda c: field_to_csv_LocationName(c.campaign_criterion),
            csv_to_field=lambda c, v: csv_to_field_LocationName(c.campaign_criterion, v)
        ),
    ]
    ...
    def process_mappings_from_row_values(self, row_values):
        ...
        self._campaign_criterion.Criterion = _CAMPAIGN_OBJECT_FACTORY_V11.create('LocationCriterion')
        self._campaign_criterion.Criterion.Type = 'LocationCriterion'
