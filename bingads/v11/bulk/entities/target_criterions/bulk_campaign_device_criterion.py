...
class BulkCampaignDeviceCriterion(_SingleRecordBulkEntity):
    ...
    * :class:`.BulkFileWriter`
    ...
    _MAPPINGS = [
        ...
        _SimpleBulkMapping(
            _StringTable.Target,
            field_to_csv=lambda c: field_to_csv_DeviceTarget(c.campaign_criterion),
            csv_to_field=lambda c, v: csv_to_field_DeviceTarget(c.campaign_criterion, v)
        ),
        _SimpleBulkMapping(
            _StringTable.OsNames,
            field_to_csv=lambda c: field_to_csv_OSName(c.campaign_criterion),
            csv_to_field=lambda c, v: csv_to_field_OSName(c.campaign_criterion, v)
        ),
    ]
    ...
    def process_mappings_from_row_values(self, row_values):
        ...
        self._campaign_criterion.Criterion = _CAMPAIGN_OBJECT_FACTORY_V11.create('DeviceCriterion')
        self._campaign_criterion.Criterion.Type = 'DeviceCriterion'
