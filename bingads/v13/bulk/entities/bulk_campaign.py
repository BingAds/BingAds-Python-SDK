...
_AISearchSetting = type(_CAMPAIGN_OBJECT_FACTORY_V13.create('AISearchSetting'))
...
class BulkCampaign(_SingleRecordBulkEntity):
    ...
    * :class:`.BulkFileWriter`
    ...
    def _get_ai_search_setting(self):
        return self._get_setting(_AISearchSetting, 'AISearchSetting')
    ...
    def _write_final_url_expansion_opt_out(c):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            return bulk_str(ai_search_setting.FinalUrlExpansionOptOut)
    ...
    def _read_final_url_expansion_opt_out(c, v):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            ai_search_setting.FinalUrlExpansionOptOut = parse_bool(v)
            return
        if 'performancemax' in campgaign_types:
            performance_max_setting = c._get_performance_max_setting()
            if not performance_max_setting:
    ...
    def _read_text_opt_out(c, v):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            ai_search_setting.AutoGeneratedTextOptOut = parse_bool(v)
            return
        if 'performancemax' in campgaign_types:
            performance_max_setting = c._get_performance_max_setting()
            if not performance_max_setting:
    ...
    def _write_text_opt_out(c):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            return bulk_str(ai_search_setting.AutoGeneratedTextOptOut)
        if 'performancemax' in campgaign_types:
            performance_max_setting = c._get_performance_max_setting()
            if not performance_max_setting:
                return None
    ...
    def _read_image_opt_out(c, v):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            ai_search_setting.AutoGeneratedImageOptOut = parse_bool(v)
            return
        if 'performancemax' in campgaign_types:
            performance_max_setting = c._get_performance_max_setting()
            if not performance_max_setting:
    ...
    def _write_image_opt_out(c):
        ...
        ai_search_setting = c._get_ai_search_setting()
        if ai_search_setting is not None:
            return bulk_str(ai_search_setting.AutoGeneratedImageOptOut)
        if 'performancemax' in campgaign_types:
            performance_max_setting = c._get_performance_max_setting()
            if not performance_max_setting:
                return None
    ...
    @staticmethod
    def _read_ai_max_enabled(c, v):
        if not v:
            return
        setting = c._get_ai_search_setting()
        if setting is None:
            setting = _CAMPAIGN_OBJECT_FACTORY_V13.create('AISearchSetting')
            setting.Type = 'AISearchSetting'
            if not c.campaign.Settings:
                c.campaign.Settings = _CAMPAIGN_OBJECT_FACTORY_V13.create('ArrayOfSetting')
            c.campaign.Settings.Setting.append(setting)
        setting.AISearchEnabled = parse_bool(v)

    @staticmethod
    def _write_ai_max_enabled(c):
        setting = c._get_ai_search_setting()
        if setting is None:
            return None
        return bulk_str(setting.AISearchEnabled)

    @staticmethod
    def _read_search_term_matching_opt_out(c, v):
        setting = c._get_ai_search_setting()
        if setting is None:
            return None
        setting.SearchTermMatchingOptOut = parse_bool(v)

    @staticmethod
    def _write_search_term_matching_opt_out(c):
        setting = c._get_ai_search_setting()
        if setting is None:
            return None
        return bulk_str(setting.SearchTermMatchingOptOut)
    ...
    _MAPPINGS = [
        ...
        _SimpleBulkMapping(
            header=_StringTable.AISearchEnabled,
            field_to_csv=lambda c: BulkCampaign._write_ai_max_enabled(c),
            csv_to_field=lambda c, v: BulkCampaign._read_ai_max_enabled(c, v)
        ),
        _SimpleBulkMapping(
        ...
        _SimpleBulkMapping(
            header=_StringTable.SearchTermMatchingOptOut,
            field_to_csv=lambda c: BulkCampaign._write_search_term_matching_opt_out(c),
            csv_to_field=lambda c, v: BulkCampaign._read_search_term_matching_opt_out(c, v)
        ),
        _SimpleBulkMapping(
